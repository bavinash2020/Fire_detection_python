...
def send_mail_function(): # defined function to send mail post fire detection using threading
    ...
    recipientmail = "9572496969a@gmail.com" # recipients mail
    recipientmail = 'arpita@edunetfoundation.org' # To lower case mail
    
    try:
        server = smtplib.SMTP('smtp.gmail.com', 587)
        server.ehlo()
        server.starttls()
        server.login("9572496969a@gmail.com", 'Avinash@123') # Senders mail ID and password
        server.sendmail('arpita@edunetfoundation.org', recipientmail, "Warning fire accident has been reported") # recipients mail with mail message
